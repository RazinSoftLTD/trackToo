...
class PayrollController extends AccountBaseController
{
    ...
    public function passAuthKey()
    {
        try{
            $this->data['pathToGo'] = decrypt(request()->path);
            return view('payroll::verify',  $this->data);
        }catch(\Exeption $e){
            return back();
        }
    }
     ...
     public function setPayrollAuthKey()
     {
        $password = request()->password;
        $userPassword = user()->password;
        $isPass =  Hash::check($password, $userPassword);
        if($isPass){
            $key = cookie('payroll_auth_key', $userPassword, 10);
